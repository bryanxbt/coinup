...
export function PlayClient({ meta }: { meta: GameMeta }) {
  ...
  const fullBleed = meta.id === "crazy-wheel" && phase === "playing";

  return (
    <main
      className={
        fullBleed
          ? "arcade-grid mx-auto flex w-full max-w-xl flex-col items-center px-2 py-4 sm:px-4"
      ...
      {phase === "playing" && session && (
        <div
          className={
            fullBleed
              ? "w-full"
              : "pixel-panel w-full border-[var(--varsity-blue)] p-4"
          }
        >
          <Play
            session={session}
